...
COL_END = (255, 0, 0) # RED
COL_IMPOSSIBLE = (127, 127, 127) # DARK_RED
...
class Item():
    ...
    def get_neighbour(self, grid, start):
        ...
        if self.row > 0 and self.row < ROWS and self.col >= 0 and self.col < COLUMNS and not grid[self.row - 1][self.col].is_wall() and not (self.row-1==start.row and self.col==start.col):
            neighbours.append(grid[self.row - 1][self.col])

        if self.row < ROWS - 1 and self.col >= 0 and self.col < COLUMNS and not grid[self.row + 1][self.col].is_wall() and not (self.row+1==start.row and self.col==start.col):
            neighbours.append(grid[self.row + 1][self.col])

        if self.col > 0 and self.row >= 0 and self.row < ROWS and not grid[self.row][self.col - 1].is_wall() and not (self.row==start.row and self.col-1==start.col):
            neighbours.append(grid[self.row][self.col - 1])

        if self.col < COLUMNS - 1 and self.col>=0 and self.row >= 0 and self.row < ROWS and not grid[self.row][self.col + 1].is_wall() and not (self.row==start.row and self.col+1==start.col):
            neighbours.append(grid[self.row][self.col + 1])
    ...
    def get_neighbours(self, grid):
        ...
        if self.row > 0 and self.row < ROWS and self.col >= 0 and self.col < COLUMNS and not grid[self.row - 1][self.col].is_wall() :
            neighbours.append(grid[self.row - 1][self.col])

        if self.row < ROWS - 1 and self.col >= 0 and self.col < COLUMNS and not grid[self.row + 1][self.col].is_wall():
            neighbours.append(grid[self.row + 1][self.col])

        if self.col > 0 and self.row >= 0 and self.row < ROWS and not grid[self.row][self.col - 1].is_wall():
            neighbours.append(grid[self.row][self.col - 1])

        if self.col < COLUMNS - 1 and self.col>=0 and self.row >= 0 and self.row < ROWS and not grid[self.row][self.col + 1].is_wall():
            neighbours.append(grid[self.row][self.col + 1])
    ...
    def draw(self):
        pygame.draw.rect(self.screen, 
			self.color, 
			(self.x, self.y, self.ITEM_HEIGHT, self.ITEM_WIDTH))
